...
public class UserTableController {
	...
	public ResponseEntity<UserTable> authenticateUser(@RequestBody UserTable userTable) {
		String email = userTable.getEmail();
		String password = userTable.getPassword();
		UserTable userDetails = userTableDAO.validate(email, password);

		session.setAttribute("userEmail", email);
		session.setAttribute("userDetails", userDetails);

		return new ResponseEntity<UserTable>(userDetails, HttpStatus.OK);
	}
